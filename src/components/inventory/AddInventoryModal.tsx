...
// ─────────────────────────────────────────────────────────────────────────────
// CATEGORY-LEVEL FIELDS  (shown for any subcategory within the category)
// ─────────────────────────────────────────────────────────────────────────────
export const CATEGORY_FIELDS: Record<string, CategoryField[]> = {
  ...
  medical: [
    ...
    { key: 'manufacturer_country',  label: 'Country of Manufacture',     type: 'select', options: ['Kenya', 'South Africa', 'India', 'UK', 'Germany', 'France', 'USA', 'Netherlands', 'Belgium', 'Israel', 'Australia', 'Other'] },
    ...
    { key: 'storage_temperature',   label: 'Storage Temperature',        type: 'select', options: ['Room temp (15–25 °C)', 'Cool (8–15 °C)', 'Refrigerated (2–8 °C)', 'Frozen (< −18 °C)', 'Protected from light', 'Other'] },
    { key: 'dosage_per_animal',     label: 'Dosage per Animal',          type: 'text',   placeholder: 'e.g. 5 ml / 100 kg BW', hint: 'Standard dose for one cow' },
    { key: 'route_of_administration', label: 'Route of Administration',  type: 'select', options: ['Intramuscular (IM)', 'Intravenous (IV)', 'Subcutaneous (SC)', 'Intramammary', 'Oral / Drench', 'Topical / Pour-on', 'Intranasal', 'Intravaginal', 'Other'] },
  ...
  cropInputs: [
    ...
    { key: 'formulation_type',      label: 'Formulation Type',           type: 'select', options: ['Granular', 'Powder / WP', 'Liquid / EC', 'Suspension Concentrate (SC)', 'Soluble Liquid (SL)', 'Seed dressing', 'Other'] },
    ...
    { key: 'target_crop',           label: 'Target Crop',               type: 'select', options: ['Napier grass', 'Rhodes grass', 'Kikuyu', 'Maize (silage)', 'Sorghum', 'Lucerne / Alfalfa', 'Brachiaria', 'Other forage', 'Vegetables', 'Other'] },
  ...
  construction: [
    ...
    { key: 'structure_type',             label: 'Structure / Asset Type',         type: 'select', options: ['Dairy parlour', 'Calf pen', 'Maternity pen', 'Feed store', 'Silage pit / bunker', 'Water tank', 'Perimeter fence', 'Crush / handling facility', 'Staff quarters', 'Office', 'Other'] },
  ...
  // ── ⛽ FUEL & ENERGY ────────────────────────────────────────────────────
  fuel: [
    { key: 'fuel_type',                   label: 'Fuel Type',                     type: 'select', options: ['Diesel', 'Petrol', 'LPG (cooking)', 'Lubricant / Engine oil', 'Hydraulic oil', 'Gear oil', 'Grease', 'Paraffin', 'Other'] },
  ...
  dairyHygiene: [
    ...
    { key: 'application_point',          label: 'Application Point',              type: 'select', options: ['Pre-dip', 'Post-dip', 'Pre & post dip', 'CIP / Pipeline wash (acid)', 'CIP / Pipeline wash (alkali)', 'Teat spray', 'Surface / floor disinfectant', 'Footbath', 'Other'] },
    ...
    { key: 'foam_or_liquid',             label: 'Product Form',                   type: 'select', options: ['Liquid (ready to use)', 'Liquid concentrate', 'Foam concentrate', 'Powder', 'Gel', 'Tablet / effervescent', 'Other'] },
  ...
  breeding: [
    ...
    { key: 'breed',                   label: 'Breed',                         type: 'select', options: ['Holstein Friesian (HF)', 'Jersey', 'Guernsey', 'Ayrshire', 'Brown Swiss', 'Fleckvieh', 'Montbeliarde', 'HF × Jersey cross', 'HF × Ayrshire cross', 'Boran', 'Sahiwal', 'Other crossbreed', 'Other'] },
  ...
  office: [
    { key: 'item_type',               label: 'Item Type',                     type: 'select', options: ['Ear tags (conventional)', 'RFID / EID tags', 'Rumen boluses (ID)', 'Rumen boluses (health)', 'Neck tags', 'Leg bands', 'Stationery', 'PPE — Gloves (examination)', 'PPE — Gloves (household)', 'PPE — Boots', 'PPE — Overalls', 'PPE — Safety helmets', 'Printer / label consumables', 'Other'] },
    { key: 'manufacturer',            label: 'Manufacturer / Brand',          type: 'text',   placeholder: 'e.g. Allflex, Dalton, Destron Fearing, Caisley' },
    { key: 'regulatory_standard',     label: 'Regulatory Standard',           type: 'text',   placeholder: 'e.g. KVB / KEBS approved, ISO 11784/11785 compliant' },
    { key: 'tag_colour',              label: 'Tag Colour',                     type: 'select', options: ['Yellow', 'Orange', 'Red', 'Blue', 'Green', 'White', 'Black', 'Pink', 'Other'] },
    { key: 'tag_size',                label: 'Tag Size / Type',                type: 'select', options: ['Full size (adult)', 'Calf size (small)', 'Button tag', 'Visual + EID combo', 'Other'] },
    { key: 'tags_per_animal',         label: 'Tags per Animal',               type: 'number', placeholder: '2', hint: 'Both ears? Official + management tag?' },
    { key: 'annual_calves_est',       label: 'Est. Annual Calves',            type: 'number', placeholder: '500', hint: 'Drives minimum annual tag stock requirement' },
    { key: 'frequency_mhz',          label: 'RFID Frequency',                type: 'select', options: ['N/A (visual only)', '134.2 kHz (ISO 11784/11785)', '125 kHz (EM4100)', 'UHF 860–960 MHz', 'Other'] },
  ],

  // ── 📦 PACKAGING / SALES ────────────────────────────────────────────────
  packaging: [
    { key: 'container_type',          label: 'Container Type',                type: 'select', options: ['20L jerry can (HDPE)', '10L jerry can (HDPE)', '5L bottle', '1L bottle', '500 ml bottle', '250 ml bottle', 'Sachet / pouch (50 ml)', 'Sachet / pouch (500 ml)', 'Bulk tank (own)', 'Carton / tray', 'Other'] },
    ...
    { key: 'label_language',          label: 'Label Language(s)',             type: 'select', options: ['English', 'Swahili', 'English + Swahili', 'Other'] },
  ...
  kitchen: [
    ...
    { key: 'meals_per_day',           label: 'Meals per Day',                 type: 'select', options: ['1 (lunch only)', '2 (breakfast + lunch)', '2 (lunch + dinner)', '3 (breakfast + lunch + dinner)', 'Other'] },
    ...
    { key: 'storage_method',          label: 'Storage Method',                type: 'select', options: ['Dry store (ambient)', 'Refrigerated (2–8 °C)', 'Frozen (< −18 °C)', 'Open shelf', 'Sealed container / bin', 'Other'] },
...
// ─────────────────────────────────────────────────────────────────────────────
// SUBCATEGORY-SPECIFIC FIELDS  (shown ONLY when a matching subcategory is selected)
// Key format:  "categoryCode:subcategoryCode"
// ─────────────────────────────────────────────────────────────────────────────
export const SUBCATEGORY_FIELDS: Record<string, CategoryField[]> = {

  // ── Medical sub-categories ───────────────────────────────────────────────
  'medical:vaccines': [
    { key: 'vaccine_type',            label: 'Vaccine Type',                  type: 'select', options: ['Live attenuated', 'Inactivated / Killed', 'Toxoid', 'Subunit / recombinant', 'mRNA', 'Other'] },
    ...
    { key: 'vaccination_program',     label: 'Vaccination Program',           type: 'select', options: ['Primary course (2 doses)', 'Annual booster (1 dose)', 'Biannual booster', 'Quarterly', 'On outbreak / ring vaccination', 'Calves only', 'Other'] },
  ...
  'medical:antibiotics': [
    ...
    { key: 'indication',              label: 'Primary Indication',            type: 'select', options: ['Mastitis (intramammary)', 'Mastitis (systemic)', 'Respiratory infection (BRD)', 'Foot rot / Digital dermatitis', 'Metritis / Endometritis', 'Navel ill (calves)', 'Pneumonia (calves)', 'Diarrhoea (calves)', 'Wound / abscess', 'Dry cow therapy', 'Other'] },
  ...
  'medical:dewormers': [
    { key: 'drug_class',              label: 'Anthelmintic Class',            type: 'select', options: ['Benzimidazoles (BZ) — e.g. Albendazole, Fenbendazole', 'Macrocyclic lactones (ML) — e.g. Ivermectin, Doramectin', 'Levamisole / Tetramisole', 'Closantel (flukes + worms)', 'Salicylanilides', 'Combination product', 'Other'] },
  ...
  'medical:hormones': [
    { key: 'hormone_type',            label: 'Hormone Type',                  type: 'select', options: ['Oxytocin', 'GnRH (e.g. Receptal, Fertagyl)', 'Prostaglandin F2α (PGF2α — e.g. Estrumate, Lutalyse)', 'Progesterone (CIDR / PRID)', 'Oestradiol', 'Dexamethasone / Corticosteroid', 'Insulin / metabolic', 'Other'] },
    { key: 'reproductive_protocol',   label: 'Reproductive Protocol',         type: 'select', options: ['Ovsynch', 'Doublesynch', 'Presynch-Ovsynch', 'G6G', 'CIDR-based protocol', 'Heat detection + AI', 'Induction of parturition', 'Milk let-down (oxytocin)', 'Other'] },
  ...
  'cropInputs:pesticides': [
    ...
    { key: 'application_equipment',   label: 'Application Equipment',         type: 'select', options: ['Knapsack sprayer', 'Motorised mist blower', 'Tractor-mounted boom', 'Drone / aerial', 'Granule spreader', 'Seed treatment dresser', 'Other'] },
  ...
  'fuel:lubricants': [
    { key: 'viscosity_grade',         label: 'Viscosity Grade',               type: 'select', options: ['SAE 5W-30', 'SAE 10W-40', 'SAE 15W-40', 'SAE 20W-50', 'SAE 30 (monograde)', 'SAE 40 (monograde)', 'ISO VG 46 (hydraulic)', 'ISO VG 68 (hydraulic)', 'NLGI 2 grease', 'Other'] },
  ...
  'maintenance:spareparts': [
    { key: 'system',                  label: 'Equipment System',              type: 'select', options: ['Milking cluster / liner', 'Milk pump', 'Pulsator', 'Vacuum pump', 'Plate cooler', 'Milk tank / bulk cooler', 'Generator / alternator', 'Tractor drivetrain', 'Water pump', 'Automatic feeders', 'Heat detection system', 'Other'] },
  ...
  'maintenance:plumbing': [
    { key: 'pipe_material',           label: 'Pipe / Fitting Material',       type: 'select', options: ['uPVC', 'cPVC (hot water)', 'Polypropylene (PP-R)', 'HDPE', 'Copper', 'GI (galvanised iron)', 'Stainless steel', 'PEX', 'Other'] },
  ...
  'maintenance:tools': [
    { key: 'tool_category',           label: 'Tool Category',                 type: 'select', options: ['Hand tools (spanners, pliers)', 'Power tools (drill, grinder)', 'Welding equipment', 'Lifting equipment (jack, chain block)', 'Measuring instruments', 'PPE / safety equipment', 'Other'] },
  ...
  'dairyHygiene:teatdips': [
    ...
    { key: 'application_method',      label: 'Application Method',            type: 'select', options: ['Dip cup (individual)', 'Spray gun / hand pump', 'Automatic post-spray system', 'Other'] },
  ...
  'dairyHygiene:disinfectants': [
    { key: 'pathogen_spectrum',       label: 'Pathogen Spectrum',             type: 'select', options: ['Bacteria only (bactericidal)', 'Bacteria + fungi', 'Bacteria + enveloped viruses', 'Broad spectrum (bacteria, fungi, viruses, spores)', 'Mycobactericidal (TB)', 'Other'] },
    ...
    { key: 'footbath_change_interval',label: 'Footbath Change Interval',      type: 'select', options: ['Every 150 cow passes', 'Every 200 cow passes', 'Daily', 'Every 2 days', 'Twice weekly', 'Weekly', 'Other'] },
  ],
  'dairyHygiene:detergents': [
    { key: 'detergent_type',          label: 'Detergent Type',                type: 'select', options: ['Alkaline (protein/fat removal — CIP acid step pre-wash)', 'Caustic alkali (strong pipeline cleaning)', 'Acid (mineral / milk stone removal)', 'Chlorinated alkali (combined clean + sanitise)', 'Enzyme-based (biofilm)', 'Neutral (teat/udder washing)', 'Other'] },
  ...
  'breeding:semen': [
    ...
    { key: 'heat_detection_method',   label: 'Heat Detection Method',         type: 'select', options: ['Visual observation', 'Tail paint / chalk', 'Kamar patches', 'Pedometer / activity monitor', 'Automated (HR-Tag, SCR)', 'Synchronisation protocol (no heat detection)', 'Other'] },
    { key: 'ai_method',               label: 'AI Method',                     type: 'select', options: ['Natural heat AI', 'Timed AI (TAI) — fixed time', 'Double Ovsynch', 'Presynch-Ovsynch', 'Other protocol'] },
    ...
    { key: 'target_calving_pattern',  label: 'Target Calving Pattern',        type: 'select', options: ['Year-round (continuous)', 'Compact (8–10 week block)', 'Spring block', 'Autumn block', 'Other'] },
  ...
  'breeding:aiconsumables': [
    { key: 'consumable_type',         label: 'Consumable Type',               type: 'select', options: ['AI sheaths / straws guards', 'Gloves (rectal examination)', 'Lubricant', 'AI gun / catheter', 'Straw cutter / tweezers', 'Thawing flask / thermometer', 'LN₂ top-up equipment', 'Synchronisation syringes / needles', 'CIDR / PRID applicator', 'Embryo transfer equipment', 'Other'] },
  ...
  'office:stationery': [
    { key: 'stationery_use',          label: 'Primary Use',                   type: 'select', options: ['Milk recording sheets', 'Health treatment records', 'Breeding / calving records', 'Feed intake sheets', 'Daily task / labour log', 'Financial records', 'General office', 'Other'] },
  ],
  'office:staffcons': [
    { key: 'ppe_type',                label: 'PPE Type',                      type: 'select', options: ['Examination gloves (nitrile)', 'Household gloves (rubber)', 'Milking overalls', 'Gumboots / wellington boots', 'Safety helmet', 'Face shield / goggles', 'Ear defenders', 'Hi-vis vest', 'Other'] },
    { key: 'size',                    label: 'Size',                          type: 'select', options: ['XS', 'S', 'M', 'L', 'XL', 'XXL', 'One size / universal', 'Other'] },
  ...
  'kitchen:drygoods': [
    { key: 'grain_type',              label: 'Grain / Staple Type',           type: 'select', options: ['Maize flour (unga)', 'Rice', 'Wheat flour', 'Beans (mixed)', 'Lentils', 'Soya', 'Rolled oats / porridge', 'Other'] },
  ...
  'kitchen:gas': [
    { key: 'cylinder_size_kg',        label: 'Cylinder Size',                 type: 'select', options: ['6 kg', '13 kg', '22 kg', '35 kg (commercial)', '48 kg (commercial)', 'Bulk tank', 'Other'] },
  ...
  'kitchen:appliances': [
    { key: 'appliance_type',          label: 'Appliance Type',                type: 'select', options: ['Industrial gas cooker / stove', 'Pressure cooker', 'Electric kettle', 'Microwave', 'Refrigerator (kitchen)', 'Chest freezer (kitchen)', 'Water dispenser', 'Other'] },
  ...
  'packaging:containers': [
    ...
    { key: 'lid_type',                label: 'Lid Type',                      type: 'select', options: ['Screw cap', 'Snap-on lid', 'Tamper-evident ring cap', 'Foil heat seal', 'No lid (open top)', 'Other'] },
  ...
  'packaging:labels': [
    { key: 'label_material',          label: 'Label Material',                type: 'select', options: ['Paper (uncoated)', 'Paper (gloss laminated)', 'Polypropylene (PP) self-adhesive', 'BOPP (waterproof)', 'Shrink sleeve (PVC/PET)', 'Direct thermal (barcode only)', 'Other'] },
    { key: 'printing_method',         label: 'Printing Method',               type: 'select', options: ['Inkjet (in-house)', 'Laser (in-house)', 'Flexographic (supplier)', 'Digital offset (supplier)', 'Other'] },
    { key: 'label_dimensions_mm',     label: 'Label Dimensions',              type: 'text',   placeholder: 'e.g. 100 mm × 70 mm' },
    { key: 'barcode_type',            label: 'Barcode Type',                  type: 'select', options: ['EAN-13', 'EAN-8', 'Code 128', 'QR code', 'No barcode', 'Other'] },
  ],

  // ── Construction sub-categories ──────────────────────────────────────────
  'construction:cement': [
    { key: 'cement_class',            label: 'Cement Class',                  type: 'select', options: ['CEM I 42.5R (OPC)', 'CEM I 52.5R (high early strength)', 'CEM II / B-V 32.5R (PPC — pozzolanic)', 'CEM III (blast furnace — sulphate resistant)', 'Special rapid-hardening', 'Other'] },
    { key: 'bags_per_m3_mix',         label: 'Bags per m³ Mix',               type: 'number', placeholder: '7', unit: 'bags/m³', hint: 'e.g. 7 × 50 kg bags for C25 mix (1:2:4)' },
  ],
  'construction:iron': [
    { key: 'steel_grade',             label: 'Steel Grade',                   type: 'select', options: ['Y8 (8mm deformed bar)', 'Y10 (10mm deformed bar)', 'Y12 (12mm deformed bar)', 'Y16 (16mm deformed bar)', 'Y20 (20mm deformed bar)', 'R6 (6mm round — links)', 'Mild steel flat bar', 'Angle iron', 'Square hollow section (SHS)', 'Rectangular hollow section (RHS)', 'GI sheet', 'Other'] },
  ...
  'construction:roofing': [
    { key: 'roofing_type',            label: 'Roofing Type',                  type: 'select', options: ['GI corrugated iron sheets (gauge 30)', 'GI corrugated iron sheets (gauge 28)', 'Box profile (IBR)', 'Colorbond / prepainted IBR', 'Fibre cement / asbestos-free', 'Polycarbonate (clear sheets)', 'Bamboo / thatch (calf pens)', 'Other'] },
  ...
  'construction:fencing': [
    ...
    { key: 'paddock_or_perimeter',    label: 'Use',                           type: 'select', options: ['Perimeter fence', 'Paddock subdivision', 'Handling facility', 'Calf pen', 'Parlour holding area', 'Other'] },
  ],
}

// ─────────────────────────────────────────────────────────────────────────────
// MachineryPicker component
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// CategoryDetailsStep — renders CATEGORY + SUBCATEGORY fields together
// ─────────────────────────────────────────────────────────────────────────────
function CategoryDetailsStep({
  ...
  const catFields  = CATEGORY_FIELDS[categoryCode]    ?? []
  ...
  // Deduplicate by key (subcat fields take precedence)
  const seen = new Set<string>()
  const fields = allFields.filter(f => { if (seen.has(f.key)) return false; seen.add(f.key); return true })
      ...
      return (
        <div
          ...
          className={`flex items-center justify-between p-3.5 rounded-xl border-2 cursor-pointer transition-all select-none ${
            val ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
          }`}
        >
  ...
  // Group: category-level vs sub-category-level
  const catOnly  = catFields.filter(f => !seen.has(f.key + '_dup'))   // all cat fields
  const subOnly  = subFields.filter(f => !catFields.some(cf => cf.key === f.key)) // only unique sub fields
...
export function AddInventoryModal({ farmId, isOpen, onClose, onItemAdded }: AddInventoryModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchDepts = async () => {
      ...
      } catch { /* silent */ } finally { setIsLoadingDepts(false) }
    }
    const fetchCategories = async () => {
      ...
      } catch { /* silent */ } finally { setIsLoadingCats(false) }
    }
    fetchDepts()
    fetchCategories()
  }, [isOpen])
  ...
  const handleClose = () => {
    setForm({
      ...
      unit_of_measure: 'kg', cost_per_unit: '', supplier: '', storage_location: '',
      department_id: '', current_stock: '', minimum_stock: '', reorder_level: '',
      reorder_quantity: '', is_perishable: false, requires_batch_tracking: false,
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 'basic' && (
            <div className="space-y-4">
              <div>
                ...
                <Input
                  id="name"
                  placeholder="e.g. Dairy Meal (High Energy), Oxytetracycline 20%, Penicillin-Strep"
                  value={form.name}
                    ...
                    {subcategories.map((sub: any) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          // Reset meta when subcategory changes
                          setCategoryMeta({})
                          setMetaErrors({})
                        }}
              ...
              {/* Machinery picker — only for maintenance category */}
          ...
          {step === 'stock' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="current_stock" type="number" min="0" step="0.01" placeholder="0"
                  value={form.current_stock} onChange={e => set('current_stock', e.target.value)} className="mt-1" />
              ...
              <div>
                ...
                <Input id="minimum_stock" type="number" min="0" step="0.01"
                  placeholder="Alert when below this"
                  value={form.minimum_stock} onChange={e => set('minimum_stock', e.target.value)} className="mt-1" />
              ...
              <div>
                ...
                <Input id="reorder_level" type="number" min="0" step="0.01"
                  placeholder="Trigger reorder at this level"
                  value={form.reorder_level} onChange={e => set('reorder_level', e.target.value)} className="mt-1" />
                <p className="text-xs text-gray-500 mt-1">
              ...
              <div>
                ...
                <Input id="reorder_quantity" type="number" min="0" step="0.01"
                  placeholder="Default quantity to order"
                  value={form.reorder_quantity} onChange={e => set('reorder_quantity', e.target.value)} className="mt-1" />
              </div>
              ...
              {form.current_stock && form.cost_per_unit && (
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-700 mt-1">
                    KES {(Number(form.current_stock) * Number(form.cost_per_unit)).toLocaleString()}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              <div
                ...
                className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ${
                  form.is_perishable ? 'border-orange-400 bg-orange-50' : 'border-gray-200 hover:border-gray-300'
                }`}
              >
                <div className="flex items-center gap-3">
              ...
              <div
                ...
                className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ${
                  form.requires_batch_tracking ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
                }`}
              >
                <div className="flex items-center gap-3">
              ...
              {(form.is_perishable || form.requires_batch_tracking) && (
                <div className="space-y-3 pt-1">
                  <div>
                    ...
                    <Input id="expiry_date" type="date" value={form.expiry_date}
                      onChange={e => set('expiry_date', e.target.value)} className="mt-1" />
                  </div>
                  <div>
                    <Label htmlFor="batch_number" className="text-sm font-medium">Batch / Lot Number</Label>
                    <Input id="batch_number" placeholder="e.g. PEN-2026-C4"
                      value={form.batch_number} onChange={e => set('batch_number', e.target.value)} className="mt-1" />
                  </div>
                </div>
              )}
          ...
          {step === 'review' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Current Stock', value: form.current_stock ? `${form.current_stock} ${form.unit_of_measure}` : '—' },
                  { label: 'Minimum Stock', value: form.minimum_stock ? `${form.minimum_stock} ${form.unit_of_measure}` : '—' },
                  { label: 'Reorder Level', value: form.reorder_level ? `${form.reorder_level} ${form.unit_of_measure}` : '—' },
                  { label: 'Reorder Qty', value: form.reorder_quantity ? `${form.reorder_quantity} ${form.unit_of_measure}` : '—' },
                  { label: 'Cost per Unit', value: form.cost_per_unit ? `KES ${Number(form.cost_per_unit).toLocaleString()}` : '—' },
              ...
              {Object.keys(categoryMeta).length > 0 && (() => {
                ...
                const subFields = subKey ? (SUBCATEGORY_FIELDS[subKey] ?? []) : []
                const seen = new Set<string>()
                const allFields = [...catFields, ...subFields].filter(f => {
