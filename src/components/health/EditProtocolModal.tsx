// EditProtocolModal for updating existing health protocols
// src/components/health/EditProtocolModal.tsx
...
interface EditProtocolModalProps {
  ...
  isOpen: boolean
  onClose: () => void
  onProtocolUpdated: (protocol: any) => void
}
...
export function EditProtocolModal({ 
  ...
  const form = useForm<ProtocolFormData>({
    ...
    defaultValues: {
      ...
      frequency_type: protocol.frequency_type || 'monthly',
      frequency_value: protocol.frequency_value || 1,
      start_date: protocol.start_date?.split('T')[0] || '',
      end_date: protocol.end_date?.split('T')[0] || '',
      target_animals: protocol.target_animals || 'all',
      veterinarian: protocol.veterinarian || '',
      estimated_cost: protocol.estimated_cost || 0,
      notes: protocol.notes || '',
      auto_create_records: protocol.auto_create_records ?? true,
      is_active: protocol.is_active !== false,
    },
  })

  // Update form values when protocol changes
  useEffect(() => {
    if (protocol) {
      form.reset({
        protocol_name: protocol.protocol_name || '',
  ...
  const handleSubmit = async (data: ProtocolFormData) => {
    ...
    try {
      const response = await fetch(`/api/health/protocols/${protocol.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to update protocol')
      }
      
      onProtocolUpdated(result.protocol)
      onClose()
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-semibold text-gray-900 flex items-center space-x-2">
            ...
            <span>Edit Health Protocol</span>
          </h3>
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              {watchedFrequencyType !== 'one_time' && (
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {getFrequencyLabel(watchedFrequencyType, form.watch('frequency_value'))}
                  </p>
                </div>
              )}
          ...
          {/* Settings */}
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="font-medium text-gray-900 mb-4">Protocol Settings</h4>
            ...
            <Button type="submit" disabled={loading} className="bg-purple-600 hover:bg-purple-700">
              {loading ? (
                <>
                  ...
                  <span className="ml-2">Updating Protocol...</span>
                </>
              ) : (
                <>
