// EditVeterinarianModal for updating existing veterinarian records
// src/components/health/EditVeterinarianModal.tsx
...
interface EditVeterinarianModalProps {
  ...
  isOpen: boolean
  onClose: () => void
  onVeterinarianUpdated: (veterinarian: any) => void
...
export function EditVeterinarianModal({ 
  ...
  const [selectedServices, setSelectedServices] = useState<string[]>(veterinarian.service_types || ['General Practice'])
  const [selectedPayments, setSelectedPayments] = useState<string[]>(veterinarian.preferred_payment || ['Cash'])
  ...
  const form = useForm<VeterinarianFormData>({
    ...
    defaultValues: {
      ...
      phone_primary: veterinarian.phone_primary || '',
      phone_emergency: veterinarian.phone_emergency || '',
      email: veterinarian.email || '',
      address_street: veterinarian.address_street || '',
      address_city: veterinarian.address_city || '',
      address_state: veterinarian.address_state || '',
      address_postal: veterinarian.address_postal || '',
      address_country: veterinarian.address_country || 'United States',
      availability_hours: veterinarian.availability_hours || '',
      emergency_available: veterinarian.emergency_available || false,
      travel_radius_km: veterinarian.travel_radius_km || 0,
      service_types: veterinarian.service_types || ['General Practice'],
      rates_consultation: veterinarian.rates_consultation || 0,
      rates_emergency: veterinarian.rates_emergency || 0,
      preferred_payment: veterinarian.preferred_payment || ['Cash'],
      notes: veterinarian.notes || '',
      rating: veterinarian.rating || 5,
      is_primary: veterinarian.is_primary || false,
      is_active: veterinarian.is_active !== false, // Default to true if undefined
    },
  })

  // Update form values when veterinarian data changes
  useEffect(() => {
    if (veterinarian) {
      form.reset({
        name: veterinarian.name || '',
        ...
        rating: veterinarian.rating || 5,
        is_primary: veterinarian.is_primary || false,
        is_active: veterinarian.is_active !== false,
      })
      setSelectedServices(veterinarian.service_types || ['General Practice'])
      setSelectedPayments(veterinarian.preferred_payment || ['Cash'])
    }
  }, [veterinarian, form])
  
  const watchedIsPrimary = form.watch('is_primary')
  const currentlyPrimary = veterinarian.is_primary
  ...
  const handleSubmit = async (data: VeterinarianFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to update veterinarian')
      }
      
      onVeterinarianUpdated(result.veterinarian)
      onClose()
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="space-y-4">
              ...
              {(watchedIsPrimary && !currentlyPrimary) && (
            ...
            <Button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700">
              {loading ? (
                <>
                  ...
                  <span className="ml-2">Updating Veterinarian...</span>
                </>
              ) : (
                <>
