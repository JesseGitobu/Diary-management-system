...
export function AnimalHealthRecords({ animalId, farmId, animals, canAddRecords }: AnimalHealthRecordsProps) {
  ...
  const loadData = async (newOffset: number = 0) => {
    try {
      ...
      // Fetch both health records and issues
  ...
  const handleCreateRecordFromIssue = async (issueId: string) => {
  ...
  const handleFollowUpAdded = async (followUpData: any) => {
    ...
    await loadData() // Refresh to get complete data
  }
  ...
  const getRecordTypeIcon = (type: string, className?: string) => {
    ...
    switch (type) {
      ...
      case 'dehorning': return <AlertTriangle className={cn(iconClass, "text-indigo-600")} />
  ...
  const recordTypes = ['all', 'vaccination', 'treatment', 'checkup', 'injury', 'illness', 'reproductive', 'deworming', 'dehorning']
  ...
  const renderTimelineContent = (record: HealthRecord) => {
    ...
    const isTimelineRecord = ['illness', 'injury', 'treatment', 'checkup', 'vaccination', 'reproductive', 'deworming', 'dehorning'].includes(record.record_type)
      ...
      <div className="space-y-4">
        {filteredRecords.length === 0 ? (
          ...
          filteredRecords.map((record) => {
            ...
            const shouldShowFollowUp = ['illness', 'injury', 'treatment', 'checkup', 'vaccination', 'reproductive', 'deworming', 'dehorning'].includes(record.record_type)
