...
async function processBatch(
  ...
  try {
    // STEP 1: Build dependency graph and topologically sort
    function sortAnimalsByDependency(animals: ImportAnimal[]) {
      const tagToAnimal = new Map(animals.map(a => [a.tag_number, a]))
      const sorted: ImportAnimal[] = []
      const visited = new Set<string>()
    ...
    console.log(`📊 Processing ${sortedAnimals.length} animals in dependency order`)

    // STEP 2: Fetch existing animals
    ...
    for (const animal of sortedAnimals) {
      try {
        ...
        if (originalTag && originalTag !== newAnimal.tag_number) {
          ...
          parentTagMap.set(originalTag, newAnimal.id)  // Also map original to id for lookups
        }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // PASS 2: Create all related records (purchases, production cycles, calf records)
    // ──────────────────────────────────────────────────────────────────────
