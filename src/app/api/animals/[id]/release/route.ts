...
export async function POST(request: NextRequest, context: any) {
  ...
  try {
    ...
    if (releaseDate > today) {
      return NextResponse.json({ error: 'Release date cannot be in the future' }, { status: 400 })
    }
...
export async function GET(request: NextRequest, context: any) {
  ...
  try {
    ...
    if (!releaseInfo) {
      return NextResponse.json({ error: 'No release information found for this animal' }, { status: 404 })
    }

    return NextResponse.json({ success: true, release_info: releaseInfo })
