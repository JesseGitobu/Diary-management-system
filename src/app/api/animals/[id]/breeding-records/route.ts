...
export async function GET(
  ...
  try {
    ...
    console.log('📋 [BREEDING-API] Fetching breeding records for animal:', animalId)
    ...
    if (recordsError) {
      console.error('❌ [BREEDING-API] Error fetching breeding records:', recordsError)
      throw recordsError
    }

    console.log('✅ [BREEDING-API] Found breeding records:', breedingRecords?.length || 0)
    ...
    if (breedingRecordIds.length > 0) {
      ...
      if (pregnancyError) {
        console.error('⚠️ [BREEDING-API] Error fetching pregnancy records:', pregnancyError)
      } else {
        pregnancyRecords = pregnancyData || []
        console.log('✅ [BREEDING-API] Found pregnancy records:', pregnancyRecords.length)
      }
    }
    ...
    const transformedRecords = (breedingRecords || []).map(record => {
      ...
      const isAutoGenerated = record.auto_generated || false
    ...
    console.error('❌ [BREEDING-API] Error in GET breeding records:', error)
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    console.log('📝 [BREEDING-API] Creating breeding record for animal:', animalId)
    ...
    const {
      ...
      expected_production_status = null
    } = body
    ...
    if (animalError) {
      console.error('❌ [BREEDING-API] Error fetching animal:', animalError)
      throw animalError
    }
    ...
    if (insertError) {
      console.error('❌ [BREEDING-API] Error inserting breeding record:', insertError)
      throw insertError
    }

    console.log('✅ [BREEDING-API] Created breeding record:', breedingRecord.id)
    ...
    if (pregnancyError) {
      console.error('❌ [BREEDING-API] Error inserting pregnancy record:', pregnancyError)
      throw pregnancyError
    }

    console.log('✅ [BREEDING-API] Created pregnancy record:', pregnancyRecord.id)

    // Update animal production status to 'served' if not already pregnant/dry
    const shouldUpdateStatus = !auto_generated || expected_production_status
    const targetStatus = expected_production_status || 'served'

    if (shouldUpdateStatus) {
      console.log(`🔄 [BREEDING-API] Updating production status to: ${targetStatus}`)
      ...
      if (updateError) {
        console.error('❌ [BREEDING-API] Failed to update animal status:', updateError)

        // ✅ CRITICAL: Rollback breeding and pregnancy records on failure
        await supabase.from('pregnancy_records').delete().eq('id', pregnancyRecord.id)
        await supabase.from('breeding_records').delete().eq('id', breedingRecord.id)

        throw new Error(`Failed to update animal status: ${updateError.message}`)
      }

      console.log('✅ [BREEDING-API] Production status updated successfully')
    } else {
      console.log('⏭️ [BREEDING-API] Skipping status update (auto-generated record)')
    }
    ...
    console.error('❌ [BREEDING-API] Error in POST breeding record:', error)
    return NextResponse.json(
